...
from utils import trans_2d_to_1d
...
def comparing_real_and_generated_data(bf_generator, batch_encoder, unet_generator, data, num_selected, epoch):
    ...
    plt_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'figure/train_visu', 'comparison')
...
def plot_loss(train_gen_loss, train_disc_loss, test_gen_loss, test_disc_loss):
    ...
    plt_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'figure/train_visu')
    if not os.path.exists(plt_dir):
        os.mkdir(plt_dir)
...
def comparing_tsne_on_test_set(bf_generator, batch_encoder, unet_generator, data, label_type, epoch):
    ...
    label = pd.read_csv(os.path.join(data_path, "TCGA_GTEx/y_data.tsv"), index_col=0, sep="\t")
...
def generate_and_save_images(model, epoch, test_input):
    ...
    plt_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'figure/train_visu', 'generate')
    if not os.path.exists(plt_dir):
        os.makedirs(plt_dir)
...
def comparing_pca_between_x_and_bfx(x, bf_generator, batch_encoder, unet_generator, epoch):
    ...
    plt_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'figure/train_visu', 'pca')
    if not os.path.exists(plt_dir):
        os.mkdir(plt_dir)
    plt.savefig(os.path.join(plt_dir, 'image_at_epoch_{:04d}.png'.format(epoch)))
    plt.close()  
